...
逐样本归一化 [0,1]）。默认取固定划分（训练 1-14+21-26 / 验证 15-17+27-28 /
测试 18-20+29-30）的**测试集**（与训练一致、无泄漏）。
...
用法:
    ...
    python3 tools/export_ai_studio_data.py --split all                    # 全部 250 个
...
DEFAULT_SPLIT_JSON = "outputs/split_fixed.json"


def load_split(split_json, split_name, src="datasets"):
    """从 split_fixed.json（或 split_seed<seed>.json）读划分，
    返回 [(digit, path), ...]（basename → 完整路径）。"""
...
def main():
    ...
    ap.add_argument("--split", choices=["test", "val", "all"], default="test",
                    help="取哪个划分（默认 test：固定划分的测试集）")
    ap.add_argument("--split-json", default=None,
                    help="划分清单路径（默认 outputs/split_fixed.json，"
                         "--split all 时忽略）")
