...
流程：
  ...
  2. 按"每类 14 训练 + 3 验证 + 3 测试"划分（固定 seed，可复现）；
...
产物（默认写入 outputs/）：
    ...
    split_seed<seed>.json  14/3/3 划分清单
    train_summary.json     汇总：各集合准确率、每类准确率、混淆矩阵、判定

用法:
    python3 tools/train_digit.py                          # 默认 seed=42, repeats=1
    python3 tools/train_digit.py --repeats 5              # 5 个不同 seed 求测试准确率 mean±std
...
def run_experiment(seed, args, items_by_digit, label_order,
    ...
    if save_artifacts:
        ...
        # ---- 划分清单 ----
        with open(os.path.join(out_dir, f"split_seed{seed}.json"), "w") as f:
...
def run_ensemble(args, items_by_digit, label_order, out_dir):
    ...
    plan = make_split(items_by_digit, VAL_N, TEST_N, split_seed)
    samples = {"train": [], "val": [], "test": []}
    for digit, files in items_by_digit.items():
        idx = label_order.index(digit)
        for f in files:
    ...
    torch.save({"state_dict": deploy_state, "meta": meta},
               os.path.join(out_dir, "model_best.pt"))
    with open(os.path.join(out_dir, f"split_seed{split_seed}.json"), "w") as f:
        json.dump(split_manifest(plan, items_by_digit), f,
                  ensure_ascii=False, indent=2)
    try:
    ...
    return {
        ...
        "digits": label_order,
        "height": args.height,
        "split": f"{VAL_N}+{TEST_N} (每类 训练/验证/测试)",
...
def main():
    ...
    print(f"数据: {len(items)} 个 CSV，每类 {VAL_N}+{TEST_N} 个样本划出"
          f"验证/测试，其余训练（{'/'.join(label_order)}）")
    ...
    print(f"产物已保存到 {out_dir}/: model_best.pt, model_final.pt, "
          ...
          f"split_seed{args.seed}.json, train_summary.json")
